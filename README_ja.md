...
- ✅ **モデル管理** - カスタムYOLOモデルの参照と読み込み
- ✅ **ログエクスポート** - 検出ログをCSVファイルにエクスポート
- ✅ **リアルタイム統計** - サイドバーで検出数とクラス別統計をリアルタイム表示
- ✅ **カメラ切り替え** - 検出中にカメラを切り替え可能
- ✅ **設定保存** - 現在の設定を保存
...
7.「モデル管理」をクリックしてカスタムYOLOモデルを参照・読み込み
8.「ログエクスポート」をクリックして検出ログをCSVにエクスポート
9.「設定保存」をクリックして現在の設定を保存

## 軌跡設定

- **最大消失フレーム数** - オブジェクトが追跡されなくなるまでの最大消失フレーム数
- **追跡IoU閾値** - オブジェクト関連付けのIoU閾値を調整（0.10 - 0.50）
- **検出信頼度** - 検出の最小信頼度（0.10 - 0.90）
