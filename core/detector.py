...
class DetectionEngine:
    ...
    def _draw_arrow(frame: np.ndarray, start: Tuple[int, int], end: Tuple[int, int], color: Tuple[int, int, int]) -> np.ndarray:
        ...
        ah = 10
        cv2.line(frame, (ex, ey),
                 (ex - int(ah * math.cos(angle + math.pi / 6)), ey - int(ah * math.sin(angle + math.pi / 6))), color, 3)
        cv2.line(frame, (ex, ey),
                 (ex - int(ah * math.cos(angle - math.pi / 6)), ey - int(ah * math.sin(angle - math.pi / 6))), color, 3)
        return frame
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        frame = self._draw_tracked_objects(frame, tracked, classes, show_traj, show_pred, traj_color, pred_color, stats)
        return frame, stats
