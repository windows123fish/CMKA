...
def _model_name_from_url(url):
    try:
        return os.path.basename(__import__("urllib.parse").parse.urlparse(url).path)
    except Exception:
        return "model.pt"
...
def save_log_row(file_path, **row_kwargs):
    # 统一中文键：时间、摄像头编号、追踪ID、类别名称、置信度、坐标X、坐标Y、框宽W、框高H、帧率、目标总数
    columns = ["时间", "摄像头编号", "追踪ID", "类别名称", "置信度", "坐标X", "坐标Y", "框宽W", "框高H", "帧率", "目标总数"]
    try:
        write_header = not os.path.exists(file_path) or os.path.getsize(file_path) == 0
        with open(file_path, "a", encoding="utf-8", newline="") as f:
            if write_header:
                f.write(",".join(columns) + "\n")
            f.write(",".join(str(row_kwargs.get(c, "")) for c in columns) + "\n")
        return True
    except Exception as e:
...
>>>>>>> Stashed changes
    ...
    def run(self, camera_id=0, on_frame=None):
...
<<<<<<< Updated upstream
    ...
    sys.exit(app.exec_())
=======
            while not self._camera_event.is_set():
                ret, frame = cap.read()
                if not ret:
                    fail_count += 1
                    if fail_count >= 30:
...
class DisableClassDialog(QDialog):
    def __init__(self, engine, parent=None):
        ...
        scroll.setStyleSheet("""
            QScrollArea { border: 2px solid #FFB6C1; border-radius: 10px; background-color: #FFF0F5; }
        """)
...
class TrackSettingsDialog(QDialog):
    def __init__(self, engine, parent=None):
        ...
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet("""
            QScrollArea { border: 2px solid #20B2AA; border-radius: 10px; background-color: #E0FFFF; }
        """)
        ...
        for name, color in trajectory_colors:
            ...
            btn.setStyleSheet(
                f"background-color: rgb({color[2]}, {color[1]}, {color[0]}); color: {'black' if sum(color) > 380 else 'white'}; border-radius: 5px; border: 2px solid #ccc;"
            )
...
class VideoThread(QThread):
    ...
    def __init__(self, camera_id, engine, parent=None):
        super().__init__(parent)
        self.camera_id = camera_id
        self.engine = engine
...
class MainWindow(QMainWindow):
    ...
    def update_stats(self, stats):
        try:
            self.stats_label.setText(
                f"帧率: {stats.get('帧率', 0.0)} | 目标: {stats.get('目标总数', 0)} | 平均置信度: {stats.get('平均置信度', 0.0):.3f}"
            )
        except Exception:
            pass
...
def run_qt():
    ...
    from PyQt5.QtGui import QImage, QPixmap, QFont, QEvent
